...
const values = [
  {
    title: 'Цель бегуна',
    description:
      'собрать все «точки активности» (у каждого бегуна она своя), при этом, не попасться ловцам, местоположение которых ему неизвестно',
  },
  {
    title: 'Цель ловца',
    description:
      'поймать бегуна, для этого у них неограниченные ресурсы и его локация. Однако, когда ловцы поймали бегуна, происходит «ротация»',
  },
  {
    title: 'Ротация',
    description:
      'один из ловцов становится бегуном а тот, что был бегуном становится ловцом. При этом ловцы «заморожены» на 30 минут',
  },
];

export const AboutProject = () => {
  return (
    <section id='about-project' className='font-custom overflow-hidden'>
      <Layout>
        ...
        <ul className='flex flex-col lg:flex-row w-full gap-y-12 lg:gap-x-12 lg:w-max mx-auto'>
          {values.map((value, index) => (
            <li
              className='p-4 border-4 rounded-3xl w-full lg:w-56 border-white flex flex-col items-center gap-y-4'
              key={index}
            >
              ...
              <ArrowIcon />
              <p className='text-center max-w-[320px] text-white'>
                {value.description}
              </p>
            </li>
          ))}
          ...
          <div className='-translate-x-5 relative max-w-64 xs:max-w-[320px] lg:max-w-[560px]'>
            ...
            <div className='absolute top-0 h-full -right-10 w-30 flex flex-col justify-around items-center rounded-2xl px-4 lg:px-8 bg-customPrimary'>
              ...
              <p className='text-xs lg:text-md max-w-[120px] lg:max-w-[240px]'>
                Достопримечательность, точка интереса, где нужно выполнить
                задание, например снять на фотографию. Эта фотография будет
                выслана остальным участникам
              </p>
            </div>
          </div>
        </div> */}
